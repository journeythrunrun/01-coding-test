...
for target in ones:
    ...
    que = [target]
    #~
    temp = 1 # - 0이 아닌 이유 : pop할 때 count(~~ print)하는 게 좋은데 append할 때 count하고 있어서 처음에 넣을 때도 count
    # - temp=0 & etc(pop?) 쪽으로 풀지마. 1대1 상황 맵핑 아님. 여러 개일 땐 다른 곳 거쳐서 count돼도 한 개 짜리일 땐 결과 달라.
    # > 뒤에 값 세개만 값이 1 큰 걸로도 한 요인 알아냄
    ...
    while (que):
        v = que.pop(0)
        # if map0[v[0]][v[1]]=='0': # 이거 때문에 값 2,1,1작게 나온듯
        #     not_append=True
        #     break
...
# - map0 변수 이름 : 실수하기 쉬우니까 amap
# - 'bfs'는 결국 빠른 완전 탐색이야. '탐색'은 for로 안 하는 게 나음
...
# - 첫 시도 [메모리 초과라뜨는데 알고맂즘도  그냥 틀렸었음]: 1) 첫 노드 방문 처리를 안 했음. 2) map0[i][j] == '1' 비슷한 코드에서 복붙할 때 제발 글자단위! == 에러발견못함
# >  while 또 안해도 이미 forfor로 완전탐색인데 while 탈출조건 설계 때문에,  forfor 두번 하면 탈출이라 시복은 동일해서 괜찮음
#   -> exist 저렇게 하면 무조건 forfor 한 번 더 하게됨
#   forfor한번이면 문제 해결되고 그 후 전체맵에 1이 없게 돼도, 그거 만들어가던 forfor에서 1나온적 있기에 while은 다음차례 때야 한번도 안 등장_탈출가능
#   2*O(n*2) 이라 빅오 똑같( exist 출력해보니 True False나옴 )

# -  필요없는 검사 회전까지 한 while : *완전엄밀히 생각하기* 엄밀끝까지 생각은 안 해봤었음.
#  (시복 큰 차이 없을 거란 계산은 안했었음 걍 시복 널널했었으니 추가 생각 시간 절약함)
#   -> 시간 급해도 알고리즘 상에선 엄밀 끝까지 생각하자.
#   > 같은 단지인지 검사 끝나고 처음부터 다시 돌아야한다고  생각했었음.
#   > 근데 이미 검사 시작 대상을 모든 곳에서 forfor로하고있음. 이건 바이러스같은거고, '시작대상'도 방문했던 곳 다시 할 필요도 없지.
