...
# 2) m) for하기엔 n이 크니까, 2진수 조건 먼저 처리해보자 <-> m2) n이 숫자가 커도 조건을 만족하는 다음 큰 n과의 간격은 크지 않을 거라 ㄱㅊ함[for 하기엔 n이 커서 시간복잡도로 문제되지 않음]
# -> 1의 갯수 
...
# 4)
# 앞으로 떙길 위치가 없다면. i가 그런 위치라면 '추가' & int화
# 1이 1개도 없을 때 : 자연수라 발생하지 않는 경우임
# n은 1,000,000 이하

# - 32m / +1점
#   + m2로는 약 3분만에 풀은듯. 시간복잡도 괜찮은 방법이었어. 주의
#   >  m2) n이 숫자가 커도 조건을 만족하는 다음 큰 n과의 간격은 크지 않을 거라 ㄱㅊ함[for 하기엔 n이 커서 시간복잡도로 문제되지 않음]