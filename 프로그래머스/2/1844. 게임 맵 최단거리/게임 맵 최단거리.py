...
#   + 이 분 visited가 없네 ~~ 내 m_x였던 map에 저장 & 조건에서 visited대신 맵값이 0보다 큰 거 . 이분은 0말고 d + 1((갔던 곳은 첫 위치 빼곤 1아닐 거라, 이동 가능한 칸인지 검사하던 1인지 체크가 미방문 체크인 것도 됨. 근데 어차피 맵값이 최종 최소보다 크면 줄여나가네. bfs라) )
from collections import deque
def solution(maps):
    x_move = [1, 0, -1, 0]
    y_move = [0, 1, 0, -1]
