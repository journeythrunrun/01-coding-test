...
# - (꼭짓점 규칙찾기 & 꼭짓점으로부터 값은 너무 specific복잡해짐. 굳이 너무 specific & 최적화 알고리즘 생각하진 말자. 복잡해져서 못풀게 되기도 함) 
...
#         for i in range(lx,rx+1): # - 내부는 안하고 모든 사각형의 모서리 위치만 값 바꿀 땐 내부를 돌기 위한 for한개는 덜 있는 상태
#             amap[i][ly]=1 
#             amap[i][ry]=1
...
#         # 내부범위 리뉴얼
    ...
    for one_rect in rectangle : # - => for lx, ly, rx, ry in 사각형
        ...
        # - 사각형 한 쪽 길이가 1일 때 amap이 백지화 안되려면 : 모서리도 색칠해줘야함 -> m_A가 안됐던 이유.
        
        for i in range(lx,rx+1): # - 실화냐 위에 for만 range안썼었다. 
            for j in range(  ly, ry+1):
        ...
        for i in range(lx,rx+1):
            for j in range(  ly, ry+1):
                if i == lx or i==rx or j == ly or j== ry :
                    ...
                    amap[i][j]=1                
    # - 사각형 사이의 내부도 내부화는 시간 걸리니까, 설계한 알고리즘에서 문제가 생길 상황이면.
    # print(amap)
    ...
    # - > [단점해결하려는 노력_#$_][_코드 구현하기 매우 복잡한 구체화조건이었음. 차라리 더 쉬운 방법 생각해서 하는 게 나았을련지]8방에서 되는거 여러놈 있을 때 ## :  두 경로에서 오른쪽이 0/1인거의 지속성 각각 유지. -> _X_ 반시계방향이든 시계방향    
    ...
    while(q): # 두경로 : 양쪽 다른 거 찾으면 처음엔 자동으로 두개로 나옴
        ...
        if x == itemX and y == itemY : # - while q  = q 없을 때까지 = bfs로 모든 경로 끝까지 탐색 & 각 값 저장. [최소값] 케이스 : [while q & 탈출조건_목표 도달 지점].(첫도달==자동 최소값상태 )
            # - & q의 요소에 저장해뒀던 distance를 최소값의 의미로 저장
            answer=count
            break
            
        for i in range(4): # 움직 가능 방향
            # - 디버깅 : ㄷ자에서 안쪽으로 맞게 먼저 가는데, 건너뛸 위험이 있던 곳의  distance가 그 안쪽으로 맞게 먼저 간거랑 같은 distance를 가지네
            #   + ! 한 위치에서 특정 위치로 이동 했으면 break해야함 동시에 두경로로 못가. (distance 저장중이잖슴)
            
            # 부모 방향의 다음 시계/반시계 방향으로 한칸 씩 가기
            i=(direction_i +counterwise*( i+1) ) %4 # - !네 방향에서 부모의 counterwise값을 사용하려고 했는데, 아래에서 첫 출발 땐 두 자식에게 다른 counterwise로 업데이트 하다가 값 순서 섞임. v[]로 사용이 나을듯. 부모노드의 값을 업데이트 하진 않을테니까. q의 요소 많아져서 인덱스가 가진 의미 헷갈리면 : v리스트, v_info리스트 두개로 나눠.  # 1, 2, 2, 1 
            nx=x+dx[i]
            ny=y+dy[i]
            ...
            if 1<=nx<=50 and 1<=ny<=50 and visited[nx][ny]==0 and amap[nx][ny]==2: 
                ...
                # - [8방] '0'검사는 외부와의 검사라 아님(특별케이스 ㄷ케이스 ). 내부와의가 맞음. [한계] 사각형 한쪽 길이 1일 때 맞는 이동 위치인데도 내부값1근처에 없음 
                # for j in range(8): # 2방에서 -> 8방 중 0 있는지
                #     if amap [ nx+check_dx[j]  ] [ny+check_dy[j]   ] == 1:
                #         can_go=True
                #         break
                
                # - 해당알고리즘 가장복잡한 예씨의 모든 선에 따져봤어야
                
                
                # - 튀어나온 모서리 해결했어도, ㄷ에서 틀린다는 건 모서리를 건너뛴다는 거임. 이동 가능 대상은 알아도, 우선순위를 모르는 상태. -> 저 시계반시계방법
                if 1: # can_go==True:
                    # - 개복잡해야하는 설정해야하는 규칙_기왕 다른 방법 생각해서 구현하자 _ count가 0인 초기값 케이스엔 두 경로가 있어서 counterwise를 부모로 받지 않고 초기화 
                    if count ==0: ## 가는 방향의 왼쪽이 0인지==가는방향에서 시계방향으로 한칸 간곳의 맵값 1임 / 0임 
                        ...
                        # - ㄷ해결을 위한 시계/반시계가 모든 케이스 커버하는 알고리즘인지. [첫시작이 코너일 때] 못하네.-> 둘다 -1 저장해버림
                        
                        
                        
                        # - [[표기 나중  #_코드 큰 의미 ##_세부 코드 의미 설명 ###_다른곳에서 적었던 내용 가져옴
                        
                        
                        # - 굳이 생각한 모든 m 을 구현하려 할 필욘 없지 오답 구체화 패쓰
                        ...
                        '''
                        temp_counterwise=1 if amap[ nx-1 ][ny]==0 else -1 # - 50 끝 경계에서 꼬이면 amap 패딩 ## 자식 기준
                        #temp_counterwise= 1 if amap [x +dx [(i+1)%4] ] [ y+ dy[(i+1)%4] ] ==1 else -1  
                        q.append( [ nx,ny,count+1, i,temp_counterwise ] )
                        
                    else :
                        q.append( [ nx,ny,count+1, i,counterwise ] )
                    visited[nx][ny]=1 # - [[visited 체크인 위치 & bfs]] : dfs는 넣을 때 해야하지만 bfs는 선입선출이라, 뱉을 때 하든 넣을 때 하든 괜찮음. 코드 한줄 더 유무 고려보다는 둘 다 넣을 때 해야 실수 안 하고 암기 편 함. # - 실수로 한 좌표 정도 후에 한 번 더 들어간다고 해도 최소값부터 나오면 while탈출이고 선입 선출인거라 결과에 큰 영향 없을 것임.  
                    
                    
                    # - 테스트 3 정답은 맞았는데 한 방향만 넣어졌음
                    #!  7,1 에서 왜 못꺾었지 그리고 q에 양방향이 안넣어지고 한방향넣어지고 끝나네 | 선입선출 | 디큐
...
#     + -> 다른 방법으로 전환 or 그 특정케이스 해결하는 규칙찾기  짓을 반복했는데 전환한 게 될 줄 알고 이전꺼로 돌아가기 어렵게 해둠
''' =>
    + '방법 전환 시' : def 함수 내에서 조금씩 수정하지 말고, 변수 한 두개 바꾸는 거여도 걍 아래에 통째로 복붙해놓고 방법 써놓자.?  <-> or m1로 바꾼부분 검색가능하게 주석해놓든. 이어서 디버깅 오류 개선 될 땐 이게 나음
    + '방법'의 아주 'mA, mAA 큰 조건흐름, [장점][단점][한계]'는 'def함수 아래'나 '위'에 쓰자.
      + 이유 : 나중에 알고리즘적으로 부족한 케이스 및 부분 발견했을 때, 다른 방법으로 전환했다가 돌아오기도 하고, bfs로 빨리 될 거 같은 알고리즘 부족 부분 해결해나가는데 그 때 뭐였는지 찾을 수 있어야함.
      + 이유 : 함수 내부에선 해당 주석 다시 찾기 어려움. 방법 전환할 때 다 주석 쓰기도 하고 지우기도 하고 코드 길어지면 설명도 중간에 많아서 
'''    
#     + 예시만 보며 그 예시에서의 규칙찾기는 후순위인 게 나을듯. 단순 예시에서는 놓치는 게 있을 수 있음
#   + bfs 느림
#   + 알고리즘 적으로 예외있는 거 구현하는 데에 시간 많이 씀. 예제에서 확실히 다 검사후?


# - 다른 방법도 같은 값의 오답이 나올 수 있음. 오류별 원인 아래에 적어두기. 오답 17_정답19 : 처음에 두 경로 중 한 개만 가버림

# - 다른 사람 풀이
    # - 방법 : 문제는 최소 거리 탐색인데 갈 수 있는 경로 대상 '2개'밖에 없음, '두' 경로 밖에 없고 특정화 가능해서 bfs 안쓰고 min 활용 가능
    #   + min ( (모서리 따라서) 도착점까지 간 거리 , (모서리 따라서) 한 바퀴 돌 때의 거리 -( 모서리 따라서)도착점까지 간 거리 ,)
...
def is_movable(cur_x, cur_y, next_x, next_y, rectangles):
    ...
    #   + <-> 내 알고리즘에서 부족했던 부분(이것처럼 모서리/내부 검색용에서 필요했던 건 아니고 이동 우선순위 구현 부분에서 필요했던 거지만 암튼) 도 이것처럼 정수가 아닌 세부적 칸화로 했으면 해결할 수 있었겠네. # ㄷ주의 난 .5대신 모서리값은 업데이트 안하고 내부만 업데이트해서 옆에서 돌려고 했음
    x, y = (cur_x + next_x) / 2, (cur_y + next_y) / 2 # - 중간값 검사 : ㄷ자 에서 건너편 모서리로 건너뛰기 방지됨. ##오 차라리 현재 위치와 다음 위치의 평균값으로 이동가능한 위치인지 검사. 반절의 위치를 다 저장할필요 없이 반절의 위치를 '검사'
    # - 사각형 모서리에 있는지 검사 : 검사 대상인x 좌표와 y좌표 중 한 좌표는 사각형의 꼭짓점 좌표 중 1개와 같고, 나머지 좌표는 사각형의 해당 좌표 범위 이내
    ...
    #     + [장점] 모서리 위치 정확히 알면 이동 가능한 곳을 알기 위해 개쌩쑈 안해도 됨 . m_A대비 이 장점을, 방법생각했을 때 머리에 스치게 말고 확실히 방법 떠올리면 장점 알 정도로 알거나 적어두기 [단점] 튀어나온 테두리 -> in적었었는데 in으로 모든 사각형 다시 재검사 해서 튀어나온 모서리인지 아닌지 확인할 수 있음. 실제로 제대로 머리에 구체화 활용을 못했음. 꼭짓점4개에서만 in 검사하지 않구 모든 검사 대상 좌표에서 모든 사각형에 대해 in 검사해도 됨. 너무 specific규칙찾기 우선아니었나. 시복 내이면 코드로 구현하기 쉬운방법 생각하기.  
    
    #   + (m_A)내부만 1저장 하면 가능한데 그럼 모서리 여부 검사를 따로 추가해야함. ㄷ 자일 때 주의. (mAA) 모서리 미업데이트_모서리 및 경로 = 경로의 옆이 내부인 1 & 자기자신 0. 구현하다가 전환 [단점] 사각형 한 쪽 길이가 1일 때 amap이 백지화돼버림 -> 모서리도 색칠해줘야함 -> (mAB) 모서리도 내부처럼 업데이트_ [단점] ㄷ자일 때 평범한 이동알고리즘에선 모서리인지 구별 못함. -> 칸 반절 더 쪼개서 업데이트는 구현안해봄.  오 차라리 현재 위치와 다음 위치의 평균값으로 이동가능한 위치인지 검사. 반절의 위치를 다 저장할필요 없이 반절의 위치를 '검사'해서 해결가능
    
    #     + [장점]초기에 맵값설정 쉬움 (일단 빠른 방법보다 그 후 코드의 단점도 고려하하고 방법 선택하여 구현) [단점] 모서리를 '내부값 옆, 근처로, 방향성으로 돌기'로 해놔서 이동 알고리즘이 개번거로워짐. 그리고 근처로 돌아도 괜찮은가?는 예외케이스 발생하기도 쉬움.규칙찾고 괜찮은지 하나하나 점검해봐야해서 구체화할 게 많아짐. m_A는 모서리2/내부1만 잘 설정해놓으면 이동 개쉬움. 엄밀하고 규칙찾을게 없음. 
    #   + -> 차라리 초반에 적었던 방법인  in 내부인지 검사를 m_A에서 이용하자가 나았을 듯. 알고리즘 머리에서 구체화하는 속도도 부족한가. 아이디어들이 난잡하게 흩어져있음. => 문제에서 필요한 요소별 아이디어로 정리?
    ...
    # - 사각형 내부인지 검사 : 내 m_B의 문제는 내부에서 "튀어나온 모서리" 였음 -> 여기선 튀어나온 모서리일지 모르는 모든 좌표에 대해서, + 전체 사각형 다 검사하며 내부인지 검사)
    #   + 여기선 탐색할 좌표마다 사각형 다 검사하긴 했는데. 내 amap에 이 아이디어를 녹이면, m_B[모서리2내부1]에서 사각형을 한 번 더 다 돌며 이번엔 모서리값은 냅두고 내부만 1화하는 걸로 해결가능 
...
def solution(rectangle, characterX, characterY, itemX, itemY):
    ...
    for dist in itertools.count(): # - 그냥 while로 break만날 때까지 cnt증가하는 거랑 똑같음. 코드 단순화. while (1) & count =~ for _ in itertools.count() 
        ...
        # - 특성화한 방법 상 한 경로만 가면 돼서, 네 방향 중 갈 수 있는 곳 있으면 바로 break
