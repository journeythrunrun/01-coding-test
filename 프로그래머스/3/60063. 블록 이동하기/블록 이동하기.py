...
def solution(board):
    ...
    dr,dc=[0,0,-1,1],[1,-1,0,0] # - ! r변화기준이니까 위/아래가 i=2,3임. 알고도 앞쪽으로 쏠려있는 값이 봐지네.dr은 앞에 네개에서만 봐야함. 앞쪽아니고.. - 아래칸(다음행)  X_위칸 아래칸 / 아래칸위칸순서 / 따지자면 r c 로는 아래, 위이긴한데 방향성 통일뒤집을 수 있긴함. / 그래도 인덱싱맞게. NN이고정됐으니. ->#$미리
    
    q=deque([ [0,1, 0 ] ])  # print(v) 확인해보니 첫값 잘못넣었네
    # answer 
    ...
    while(q):
        ...
        #~#~~ㄹㅇ대충
        answer=v[2]
        #$#$ 두 좌표의 위치가 바뀌어도 같은 놈이어야함. 실제론 왼쪽 오른쪽 구별 없음. 그래도 맞게 나와야하는거아닌가.
        ...
        for i in range(4):
            ...
            #$ 왼쪽오른쪽 순서바꿔도 됨 ? _근데 어차피 시작할때 다른 좌표라 처음에 다른 의미 가지고 시작해서 이동할 때 구별 ㄱㅊ. 다만 조건에서 //  _*2라 뭐 굳이? 조건 따질때 ==으로 하진 않았는지.  이동이 +1되지 않도록 in 으로.

            
            
            
                
            if 0<=r1<n and 0<=c1<n and 0<=r2<n and 0<=c2<n and (visited[n*r1+c1][n*r2+c2]==0 and  visited[n*r2+c2][n*r1+c1]==0  ) : #$ - 코드 수정 시:vsiited 저장 다 바꾸는 것보다는 검사하는 곳 한 곳바꾸는 게 편함 # 미방문시
                if not board[r1][c1] and not board[r2][c2] : ## (2) amap의 값. 벽아니어야하지 
                    # 방문가능. 먹을 때 
                    ...
                    # 너무 케이스를 나눴나. 암튼 이미했음 #$ 더 간단한 회전은?
                    
                    elif not width and i==0: # 오른
                        #$#$#$ 회전시 가로케이스에서도 왼쪽오른쪽보다는 작은값으로부터. 이동하면서 회전 됐을 수 ?
...
# - 주석 빼면 1h 5m정도? : 시간오버로 효율을 위해 다른 사람 풀이 봄
...
#   + 복잡한 3차원 [visited] 대신, [딕셔너리의 키에다 좌표를 튜플형]으로 저장해서 in으로 찾으면 O(1)
