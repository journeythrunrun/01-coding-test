...
def solution(begin, target, words):
    # - bfs 시복 
    #   + 원래 기본형은 O(n) 맞는데, 이 문제 조건은 그 O(n)개에서 다른 word들[O(*n)]과 연결된 애인지를 체크(=조건에 부합하는지)해야해서(연결맵없어서) [~~탐색만 하고 미방문 했던 애들과 다시[O(*n)] 여러번 조건 탐색(O(*m))하게 돼서] O(n) 아님. 
    
    # - set으로 하면 in은 괜찮아도 방문체크 귀찮음. 미방문 됐던 애들만 add하는 set으로 이용하는 방법도 있긴함_굳이
    # set_words=set(words) 
    ...
    # - while이 더 깔 끔 : <-> m_x1미방문만 인 거 엄밀히 생각하기 전인 코드 방법_ 코드 상 반복문&new_words의 for문&탈출문 보다.((시복은 비슷할 듯)) 
    ...
    while(new_words): # 1회씩
        ...
        #     > 굳이 연결관계 체크 안 해도 되는 word들끼리 안하게 돼서 맵 미리 만든 것보다 시간복잡도 더 나은 거 아닌가
        ...
        for i, word in enumerate(words): # O(n)  & 방문했던 애는 아래에서 바로 continue
            ...
            warn=False # 위의 내용을 위한 1,2회관련 체크
             # - 시복 알고도 다시 또 어라 하기도 하네. ! 아니고 n은 두번만 곱해짐. 경로 마냥 단 두발자국 아니고 연속해서 가긴 하는데, 그 경로를 미방문만 가면서 데이터 저장해서 넘겨주기에 n(:첫발지칭)*n(두발)으로 감. 직렬의 병렬화 느낌 조금 난다랄까. ( 해당경로로 dfs로 간다기보다 bfs로 해당 애를 추가해줌(두번째 발자국 후에 바로 가지 않고 첫번째 발자국에 넘겨줌 ) 그 애가 나중에 첫 발자국으로서 앞 O(n)에 포함됨)
            ...
            new_words.append( [word, go[1]+1  ] ) # - bfs 최소값 때 : 부모에서 +1 하기 위해 디큐의 마지막 요소에 거리 넣어줌
            visited[i]=True
    # 아직도 return 못했으면 못찾은 거임
    return 0
# 4)
# 단어의 길이 3~10. 
...
#   > 코테 시간 : 슬슬 문제 당 40m 컷으로 낮춰야?
#   + 시복 빠르게 계산 못해서 시간 꽤 걸린듯. 출발~도착 리스트 헷갈리지 말기. 둘다 words에서 시작해서 이름 헷갈릴 수 있으니 start~arrive도 좋을듯


# - 5) 중간 출력 확인 : 시간 남으면.

# - visited : 굳이 연결리스트 안 만들고 for in 에서 탐색하면서도 visited이용하면 시복 꽤 줄여짐. O(n)~_ for 안 에서 연결될 애들을 조건으로 찾으며 탐색해나가는 거로 대체 

# - 다른 사람 풀이(0.85ms)_내께 더 빠름(최대 약 0.47ms) 근데 난 코딩 시간을 줄여야함 
#   > 신성한 , malza321 , 김다함 , 한충신 외 11 명
#   + 제너레이터 : 이터레이터를 생성해주는 함수, yield 사용 [https://dojang.io/mod/page/view.php?id=2412]
#     + ~~ 함수 다 돌렸을 때 yield 됐던 순서대로 for에서 나옴
...
#   + 풀수 잇는지 생각할 수 있는 법 : ~~ (1) 방문, n에서 탐색 (2)중복방문 안해도됨  (3) 연결, ~ 맵 (4)!되버리려나 (5) 최소, 출발, 도착  (etc) 문제 풀어가다가 익숙 습득 

# - [시복 계산]!중요!!!! 미방문만 갈거라 n!아님
#   + 문제 케이스보면서 뇌로 논리 돌리기보다, 코드 짰으면 코드에서 계산하는 게 빠름. 짜기 전에 계산해야 하긴함. 그럼, O(출발_bfs_n[중요. 실행 순서는 while & bfs로 중간중간 이어도 n개임. ] *n나머지 *m)
...
#   + 추가 : worst case도 큰 껍질에서 n + n-1 + n-2 + .. 관련이지 n! 아님 . n! 이려면 *인 거라 모든 word들로 이동도 가능한 상황의 경우의수여야함. ((=*이려면 모든 걸로 건너갈 수 있을 때 또 다시 했던 거 빼고 모든 걸로 넘어갈 수 있어야함))

#   + m_한글자다른것들의 map지도화(가능한 방법. 그방 법도 시복 비슷함)_words연결리스트생성 -> bfs 미니멈) : 생성 n! 아님. n^2 *m임.
