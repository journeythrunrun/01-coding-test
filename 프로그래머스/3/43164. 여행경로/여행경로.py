...
#     + 암튼 sort wortcae로 치면 nlogn 또 다른 사람풀이랑 시간 뭔가 비슷하고. 딕셔너리 내부 sort로 하면 최대 nlog n/2 . # 문제 조건에 더 fit한 낮은 값있는진굳이계산안함 걍 티켓이 계속 서로가리키는 케이스정도생각해봄. 저거 이하라는 정도만. GPT한테 물어보니까 값다 똑같으면 O(n) 이라함 굳이 여러 곳확인까진 안함

# - !!! 주의. 제출 후 채점하기 누른다고 커밋되는 거 아니고 체크 표시 확인! 나갔다가 다시 들어와야함.
