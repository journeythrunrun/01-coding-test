...
def dfs(target, dict1, answer, len_t , real_answer): # - 길이인 len_t를 건네는 이유 : len(dict1)으로 하기엔 전체 티켓 개수와 다른 경우가 있음_동일한 출발지로부터의 티켓들이 있을 때
    ...
    if len(answer)== len_t +1: 
        ...
        return 1 # - real_answer # - 함수가 사라질 때 그 함수에서 사용된 지역 변수도 사라지기 때문에 bfs 다시 올라갈 때( dfs를 함수로 구현 시 함수 탈출하며 자동으로 올라감.) 변수 계속 건네줘야함. 리스트 =으로 지역변수 새로 만드는 거 아니고 주소참조받은 걸로 요소 변경 하면 안 건네줘도 되긴함.
    ...
    for index, airport in enumerate( dict1.get(target,[]) ): # 'ICN', 'AAA' # dict1[target]가 list인 거고 그 안  요소 각각은 한 개
        ...
        # - 리딕셋 mutable은 다른 dfs에서 값가져다 쓸 때 다 동기화 돼버림 : 값 복사하면 O(n) 더드니까, remove/insert(동일한 티켓을 가진 벨류값 내에서라서 O(n)추가 아님) & append/pop 
        ...
        # - 디버깅하다가 오히려 더 이상하게 수정하기도 하네. 무조건적으로 다른 m으로 구현보다, 디버깅보며 놓친 부분을 수정. 아니면 이전방법으로 돌아갈 수 있게 하기.
        ...
        #     + 이유 : dfs가 끝까지 가서 answer= 정답에 이르고 다시 위층으로 돌아오면서, pop하기에 찾았던 정답을 반환해가버림. (리스트는 주소참조지만, 복사해서 new_answer로 건넸었음. 정답에 닿았을 때 _X_answer_can_X_=new_answer하면 [동기화]돼서 한놈만 pop해도 두 놈 다 pop된 결과뜸) 함수가 반환한 answer를 can(real_answer인 자리에 최종 노드 위치에서의 복사 없이 있던 변수)이 저장하고 있지만 리스트라 주소 참조라서(반환한 주소가 같음) answer에서 pop할 때 can도 같이 pop돼버림 
        #     + m_string이용 :   마지막에 문제에 맞게 list화 해야하긴 하지만 총 O( _ +n )이라 곱하기 아니고 상쇄됨 | 대신 return1말고 함수스택 올라오면서 return str1
        #     + m_real_answer 변수 추가.(했음) + 위로 챙겨줄 필요 없어졌기에 return할 주소 일부를 answer대신 1로 변경. 근데 리스트 길어도 어차피 주소 return이라 answer return도 그렇게 안복잡했겠네. 그래도 내 방법에서 불필요한 부분을 1로 바꿈 &복사제거& 성공=맞게 이해하고 있음.
        #     + m_글로벌변수추가
        ...
        if possible : # - if 해야하는 이유는?   -->  real_answer 안 닿았으면 for의 다음 요소 돌아야하고 닿았으면 탈출해야하기에 여기에 있어야하는 조건임
            ...
            return real_answer[0] # - for 밖 말고 여기서 return 이유 :  for문 끝까지 돌았는데 정답 길이에 못 닿았다면 즉 여기서 리턴을 못했었던 밖은 틀렸다는 것임. 그래서 for 밖은 정답이 아닌 상황인 거임. 여기 말고 for 밖에서 함수가 끝나면 자동으로 None return해서 정답찾은 것과 구별해줌. # - ! 처음엔, break해서 for 탈출하면 어차피 함수 마지막라인 이라 자동 return돼서 break라는 코드 한 줄 줄일겸 여기에 리턴한줄  ## 제대로 된값_정답(스택인 함수탑이 탈출될 때는 정답에 닿았거나 정답에 못닿고 거슬러올라올 때임. ) 리턴한 적 없으며 else : 정답아님
...
#   + 만든 딕셔너리 사전을 기준으로, 그걸 자식 노드로 하는 bfs. bfs~ 연결노드맵생각


#   + 공부할겸 다른 사람 코드보다 복사부분 없앤 O(n)화? 코드 만들었댱 : 그럼 나의 시복은 sort부분이되는데 , 전체 sort_nlogn말고 중복 출발지에 대한 sort 시복은 케이스마다 다를 것 같음
#     + 암튼 이전 시복의 *n있던걸 줄인 건 맞지만 다른 두번째로 큰 항의 상쇄가 없어지고 살아나게돼서 최종 시복이 *n줄여지는 게 바로 결정되는 건 아님 ## n**2을 n으로 줄일거면 n**2부분 말고 상쇄 지워진 항도 살펴봐야함. 
#     + 암튼 sort를 nlogn이라하면 또 다른 사람풀이랑 시간 뭔가 비슷. 딕셔너리 내부 sort로 하면 최대 nlog n/2인데 값이 다 똑같은 케이스임. # 문제 조건에 더 fit한 낮은 값있는진굳이계산안함 걍 티켓이 계속 서로가리키는 케이스정도생각해봄. 저거 이하라는 정도만. 
#       + GPT한테 물어보니까 값다 똑같으면 sort O(n) 이라함 굳이 여러 곳확인까진 안함. 그리고 길이는 덜 worst가 된 경우에  몇 개 더 같지 않은 값 됐을 때 sort에 어떤 영향 미치는지는 확실히 모름

# - !!! 주의. 제출 후 채점하기 누른다고 커밋되는 거 아니고 체크 일정시간 돌아가는 거 확인! 나갔다가 다시 들어와야함.
#   + 이런식으로 커밋 안된거 몇개 더 있을듯.. 나중에 확실히 최종 코드보려면 문제링크도 들어가봐야할듯 근데 그러면 이번처럼 수정 추가된 내용이 검색 안되긴할거임.. 

# - 저기 dfs_for내에서 '딕셔너리' 복사 시 문제의 총 시복은?  코테용으론 이것저것 빨리 알 수 있어야. ->     O(n**2 이상인거 맞음) answer에 한 요소 추가할 때마다 티켓 다 담긴 딕셔너리 복사하므로. 

#   + answer 복사 시 시복은?      1+2+..+n  덜직관적이지만 n+..+n처럼 n**2임. n(1+n)/2  ->  아래 코드 다른 사람 풀이 실행시간 보니까 나랑 시복 크게 차이 안나네. 테스트 케이스가 4개 밖에 없어서 다른 거에서 시간 절약하신 부분일까 answer복사는  매번 n 까진 아니고 dfs로 1, 2, 3 으로 복사해나가서 worstcase아니고는 꽤 절약되는 부분들이 있나 ?
