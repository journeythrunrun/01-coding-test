...
#   + 바다에서 녹임. 녹인 빙하의 값이 딱 0이면 해당 빙하 위치를 빙하에서 제거하고(리스트형 빙하에서 remove_항상remove하는 건 아니니까 빙하길이*빙하길이도 아니라 생각했었음. 각 빙하마다 한번 완전히 제거되면 n+...+1 이라 크게는 n*n) 바다에는 추가함  / ttemp등의 [0],[1]인덱스는 바다리스트에서 다음 바다리스트를 만들 때서로를 temp로 번갈아 쓰는 것과 비슷(그래서 %2하며 초기화)  
