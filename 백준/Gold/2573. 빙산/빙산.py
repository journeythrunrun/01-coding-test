...
#   + 내껀 python으로 해도 시간 초과 안나긴하네. 근데 pypy(480ms)가 훨 빠르다.
...
#   + 바다에서 녹임. 녹인 빙하의 값이 딱 0이면 해당 빙하 위치를 빙하에서 제거하고(리스트형 빙하에서 remove_항상remove하는 건 아니니까 빙하길이*빙하길이도 아니라 생각했었음. 각 빙하마다 한번 완전히 제거되면 n_((문맥상여기선빙하길이로사용중인거알수있음. 어차피 제출용으로 쓰는 거 아니고 내 공부용이라 제대로 원서에 제출한 적도 없는 링크인데 굳이 시간을 많이 써야하나? 코드에 필기 개많이 하는데 그걸 다 남들이 알아들을 수 있는 거에 하기 위해 시간 쏟는다? 의무도 아니고 봉사자도 아니니까 내가 원하는 만큼만~_컴공과아니라 이정도는 제출하면 오히려 -되는지 +되는지도 전혀 모르고, 당장 기록 예쁘게 하는 거에 시간 써서 예쁜 정리로 +받기보단 (타과라 더)코앞에 있는 컴공적 중요한 것들이 그득하다..흙))+...+1 이라 크게는 n*n. 하지만 #years에는 안 곱해지고 여러 years에 거쳐서 n*n이라, 그 for 내에 있어도 시복으로는 바깥으로 나가서 +로 연결됨) 바다에는 추가함
